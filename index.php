<?php include("_/inc/head.php"); ?>

		<title>Flourish with Liz Barrett</title>
		<meta name="description" content="..." />
		<meta name="keywords" content="..." />

	</head>

<body>
<div id="fb-root"></div>
<script>(function(d, s, id) {
  var js, fjs = d.getElementsByTagName(s)[0];
  if (d.getElementById(id)) return;
  js = d.createElement(s); js.id = id;
  js.src = "//connect.facebook.net/en_US/all.js#xfbml=1&appId=763971193627319";
  fjs.parentNode.insertBefore(js, fjs);
}(document, 'script', 'facebook-jssdk'));</script>
		<div class="main-cover">
		<?php include("_/inc/nav.php"); ?>

			<div class="row-bottom">

			<div class="row">
				<h1 id="headline" class="editable hero-headline">
					<span>The body achieves </span><span>what the mind believes.</span>
				</h1>
			</div>

			</div>

		</div>

			<div class="row intro">
				<div class="colspan12-6 colspan6-3 colspan2-2 as-grid">
					<div id="welcome" class="editable welcome overlays">
						<h1>Welcome! I’m Liz Barrett.</h1>
						<p>Thank you for coming to my website! I hope to inspire you to try new recipes, a new workout or work with me as your coach. My passion is supporting and encouraging men and women to embrace their incredible bodies and their health by showing them how good food, consistent exercise and mindset is the recipe to living joyfully for years to come. </p>
						</div>
					</div>
					<div class="colspan12-6 colspan6-3 colspan2-2 as-grid">
						<div class="feed overlays">

				<!-- Pulling in Facebook page id:'524500907614152',
		access_token:'763971193627319|IAUf3wjhHJoSZQLB2tRdI71AacA' -->
		
		<section class="fb">
			<header>
				<h1><a target="_blank" href="#">Facebook Feed</a></h1>
				<div class="fb-like" data-href="https://www.facebook.com/pages/Servicemaster-of-North-Idaho/524500907614152" data-layout="button_count" data-action="like" data-show-faces="true" data-share="false"></div>
			</header>
			<div id="wall" class="facebookWall"></div>
		</section>

						</div>
					</div>
			</div>
<div class="editable" id="features-left">
		<div class="row repeatable feature">
			<div class="colspan12-4 as-grid">
				<img src="http://www.placehold.it/600x450" alt="">
			</div>
			<div class="colspan12-8 as-grid typeset with-gutter">
				<h1>Featured Recipe</h1>
				<h2>Greek Turkey Sliders</h2>
				<div class="snippet">
					<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Sed, id, vero, commodi ut aperiam corporis nisi laudantium explicabo quis consequatur debitis eligendi quasi consequuntur asperiores deserunt placeat eos dolor ipsa.</p>
					<a href="#" class="readmore">Read More</a>
				</div>
			</div>
		</div>
</div>
<div class="editable" id="features-right">
		<div class="row repeatable feature">
			<div class="colspan12-8 as-grid typeset">
				<h1>Healthy Lifestyle Tips</h1>
				<h2>Workout Success</h2>
				<div class="snippet">
					<p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Sed, id, vero, commodi ut aperiam corporis nisi laudantium explicabo quis consequatur debitis eligendi quasi consequuntur asperiores deserunt placeat eos dolor ipsa.</p>
					<a href="#" class="readmore">Read More</a>
				</div>
			</div>
			<div class="colspan12-4 as-grid with-gutter">
				<img src="http://www.placehold.it/600x450" alt="">
			</div>
		</div>
</div>
<div class="editable" id="features-left">
		<div class="row repeatable feature">
			<div class="colspan12-4 as-grid">
				<img src="http://www.placehold.it/600x450" alt="">
			</div>
			<div class="colspan12-8 as-grid typeset with-gutter">
				<h1>Exercise Tips</h1>
				<h2>HIIT: high intensity interval training</h2>
				<div class="snippet">
					<p>HIIT, in short for High Intensity Interval Training has become very popular in the last year or so, and in my opinion for good reason.  The main reason I love HIIT, is I can give my clients a workout in very little time that will burn as much fat and calories fat and calories as a long steady 45 minute cardio workout.</p>
					<a href="#" class="readmore">Read More</a>
				</div>
			</div>
		</div>
</div>

		<?php include("_/inc/foot.php"); ?>